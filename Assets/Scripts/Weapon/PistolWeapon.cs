...
public class PistolWeapon : WeaponLong
{
    public override GameObject GenerateBullet(Vector2 dir)
    {
        ...
        GameObject bulletGo = GameManager.Instance.GetBullet("pistol");
        Bullet bullet;
        if (bulletGo == null)
        {
            bullet = Instantiate(GameManager.Instance.pistolBullet_prefab, transform.position, Quaternion.identity)
            .GetComponent<Bullet>();
            bullet.isPool = false;
        }
        else
